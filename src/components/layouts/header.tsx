      ...
      <nav className="hidden md:block">
        {navLinks.map((link) => (
          <Link
            key={link.id}
            href={link.href}
            className="group font-heading rounded px-3 text-base text-foreground hover:text-primary duration-300 transition-all ease-in-out font-semibold uppercase"
          >
            {link.label}
          </Link>
        ))}
